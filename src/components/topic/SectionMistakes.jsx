import React from 'react';
import { AlertOctagon, XCircle } from 'lucide-react';
import { CodeBlock } from '../ui/CodeBlock';

export function SectionMistakes({ mistakes }) {
  return (
    <section className="space-y-6 text-left">
      <div className="border-b border-[#E2E8F0] dark:border-[#232D3F] pb-4">
        <h2 className="text-xl font-bold text-[#0F172A] dark:text-[#F1F5F9] flex items-center gap-2">
          <AlertOctagon className="w-5 h-5 text-[#E11D48] dark:text-[#FB7185]" />
          SECTION 8 — Common JavaScript Pitfalls & Beginner Mistakes
        </h2>
        <p className="text-xs text-[#64748B] dark:text-[#94A3B8] mt-1">
          JavaScript array gotchas that ruin live coding interviews and how to prevent them.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {mistakes.map((m, idx) => (
          <div key={idx} className="p-5 rounded-md bg-[#FFFFFF] dark:bg-[#151B26] border border-[#E2E8F0] dark:border-[#232D3F] space-y-3 flex flex-col justify-between shadow-xs transition-colors">
            <div className="space-y-2">
              <h3 className="text-sm font-bold text-[#E11D48] dark:text-[#FB7185] flex items-center gap-2 font-mono">
                <XCircle className="w-4 h-4 text-[#E11D48] dark:text-[#FB7185] shrink-0" />
                {m.mistake}
              </h3>
              <div className="text-xs sm:text-sm text-[#0F172A] dark:text-[#F1F5F9] space-y-2">
                <p className="text-[#64748B] dark:text-[#94A3B8]">
                  <span className="font-semibold text-[#0F172A] dark:text-[#F1F5F9]">Why It Happens: </span>
                  {m.whyItHappens}
                </p>
                <div className="p-3 rounded-md bg-[#ECFDF5] dark:bg-[rgba(16,185,129,0.1)] border border-[#A7F3D0] dark:border-[rgba(16,185,129,0.35)] text-[#0F172A] dark:text-[#F1F5F9]">
                  <span className="font-bold text-[#059669] dark:text-[#34D399] font-mono block mb-0.5">How to Avoid:</span>
                  {m.howToAvoid}
                </div>
              </div>
            </div>

            {m.codeSnippet && (
              <CodeBlock code={m.codeSnippet} title="Comparison Code" />
            )}
          </div>
        ))}
      </div>
    </section>
  );
}

